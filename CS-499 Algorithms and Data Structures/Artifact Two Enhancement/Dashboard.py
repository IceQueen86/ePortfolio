...
## Enhancement 2: Import your custom recursive QuickSort sorting routine
...
app.layout = html.Div([
    ...
    dash_table.DataTable(id='datatable-id',
                        ...
                        ## Enhancement 2: Changed sort_action to 'custom' to intercept data handling path
                        sort_action = "custom",
                        sort_mode = "single",
                        sort_by = [],
...
## Enhancement 2: The callback now listens to BOTH the rescue radio filters AND table column sort commands  
...
def update_dashboard(filter_type, sort_by):
    ...
    ## Enhancement 2: Phase 1: Apply Filtering Routines Based on Selected Group Parameters    
    ...
    ## Enhancment 2: Transform current running dataframe block into a standard Python List of Dictionaries
    records_to_process = dff.to_dict('records')

    ## Enhancement 2: Phase 2: Intercept and route list arrays to your custom QuickSort method if a sort header is clicked
    if sort_by and len(sort_by) > 0:
        sort_field = sort_by[0]['column_id']
        is_descending = sort_by[0]['direction'] == 'desc'
        
        ## Enhancment 2: Execute your custom O(n log n) recursive parsing loop algorithm
        sorted_list = quicksort_records(records_to_process, sort_field)
        
        ## Enhancement 2: Invert array order path seamlessly if the user clicked the column header twice for descending order
        if is_descending:
            sorted_list.reverse()
            
        return sorted_list
        
    ## Enhancement 2: Return basic filtered records if no custom column sorting commands are active
